...
- **The olivetti footprint/mode desync (`cm/session--reenable-olivetti`).** easysession persists the workspace via `frameset-save` → `window-state-get`, which faithfully captures each window's `margins` and `fringes`. `olivetti-mode`'s entire effect *is* those window properties, but it is a buffer-local minor mode that nothing re-enables on restore (deliberately hooked into no major mode; easysession restores buffers by re-visiting files, re-running only the major mode + its hooks). So a restored olivetti buffer wears the **costume** — centered 2/3 column, no bands — while `olivetti-mode` is **off**: `C-c T o` then turns it *on*, and worse, with no olivetti hook on `window-size-change-functions` the column no longer tracks resizes (drag the frame narrower and the text is crushed against frozen 38-col margins). Fix: an `easysession-after-load-hook` (`cm/session--reenable-olivetti`) walks restored windows and, for any wearing the footprint (positive left margin, mode off — see `cm/session--olivetti-footprint-p`), calls `(olivetti-mode 1)`. This repairs the **cause** (re-run the mode → it recomputes geometry for the *current* frame size and reinstalls its resize hooks), not the **symptom** (never re-applies margins by hand). Detection leans on "only olivetti sets window margins in this config" (diff-hl uses fringes; the table/code narrowers use `narrow-to-region`); if a future package also sets window margins, tighten the predicate. This is the general lesson — *a frameset persists **derived** window state, so any buffer-local minor mode that mutates window geometry and isn't tied to a major mode will desync across restore; restore the source (the mode), never re-persist the derivation.* Pure predicate is ERT-tested (`tests/`).
...
**Why `edit-indirect` is declared as its own `use-package` block** (right above the `markdown-mode` block) even though we never invoke its commands directly: `markdown-mode` treats `edit-indirect` as a *soft* dependency. `markdown-edit-code-block` `require`s it lazily and, if absent, falls back to a `package-install` prompt — which uses package.el, not straight. On any machine where some other package hasn't already transitively loaded `edit-indirect`, the user gets prompted at first `C-c '` on a code block ("Package edit-indirect needed to edit code blocks. Install it now? (y or n)"). Declaring it explicitly puts the install under straight's control, so the prompt never fires. Don't remove that `use-package` block — it's load-bearing for the `C-c '` dispatch even though grep won't show any code referencing the package directly.
